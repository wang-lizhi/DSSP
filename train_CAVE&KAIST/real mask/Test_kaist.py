...
noiseSigma = 0.0
PhaseNumber = 11
model_dir = 'HSIRecon_cvpr2019_cave_%dPhase' % (PhaseNumber)
out_filePath = './Result/CVPR2019_KAIST/'
block_size = 256
channel = 28
batch_size = 64
...
for ch in range(channel):
	Cu_input[:,:,ch] = np.roll(T, shift=-ch, axis=1)
Cu_input = np.expand_dims(Cu_input, axis=0)
...
for img_no in range(ImgNum ):
    ...
    Hyper_image = testData['img']
    patch_image = testData['img']
    ...
    for ch in range(28):
        xoutput[:,:,ch] = np.roll(xoutput[:,:,ch], shift=-ch, axis=1)
    xoutput = np.expand_dims(xoutput, axis=0)
    ...
    print("loss sym is %.4f" % (cost_all_value))
